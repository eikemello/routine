...
- **`WidgetCombinedProvider`**: The combined water + meal card on the home screen (`layout/widget_water_meal.xml`, registered by `xml/widget_water_meal_info.xml`). Both sections keep the same pattern: the icon alone on the left, with the progress component, the numbers and the button row grouped in the same box on its right, separated by a view line. The water header shows today's total against the daily goal (green once the goal is reached) and a progress bar, above the three quick-add buttons with the values configured in Config. The meal header shows the meal icon, four segments - one per daily slot (Breakfast, Lunch, Tea, Dinner, left to right) - filled with the status color of the meals already logged today (translucent while the slot is empty), and the day's `x/4` count on the right (green once all four slots are logged); below sit the Correct / Warning / Wrong status buttons as a weighted row like the water buttons. A widget runs in the launcher, so there is no click listener: each button is a `TextView` with a `PendingIntent` broadcast to the provider carrying its index or status, and the water amount is read from the config when the tap arrives (never a stale value). Adding water goes through `HomeCardWaterService` and logging a meal through `HomeCardMealService.saveQuickMeal()`, the same services behind the home cards (the meal slot is the one matching the current time of day, and a slot already logged today is updated in place, keeping its observation - the "meal already exists" prompt of the dialog has no widget equivalent). The static `refresh(context)` repaints every placed widget and is called by the provider itself (after water is added or a meal is saved), by `HomeActivity` (resume, water added, meal saved, history panel closed) and by the config dialogs; `updatePeriodMillis` (30 min, the smallest interval allowed) also keeps both sections fresh, which is what rolls them over to the new day.
